...
# Loading data ----
market <- read.csv('Ch8_global_market_data.csv')
...
server <- function(input, output) {
  ...
  output$cluster_viz <- renderPlot({
    ...
    if (input$cluster_method == "Hierarchical") {
      ...
      dendrogram_color_scheme <- RColorBrewer::brewer.pal(n = 10, name = "Set3")
      
      #dendrogram_color_scheme = c("red", "blue", "green", "orange", "purple", 
